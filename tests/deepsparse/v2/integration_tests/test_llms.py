...
class TestsIntegrationLLMsPipelines:
    ...
    def get_pipeline(self, **kwargs) -> Pipeline:
        ...
        if not kwargs:
            if self.default_pipeline is None:
                self.default_pipeline = TextGenerationPipeline(**self.default_pipeline_kwargs)
    ...
    def setup(self, params_dict, max_new_tokens, internal_kv_cache, task):
        ...
        # TODO: For now we only support text generation
        # Also, for now we do not have a registry of pipelines
        # We create them on the "per-class" basis
        if task != "text-generation":
            pytest.skip("Only text-generation task is supported for now")
