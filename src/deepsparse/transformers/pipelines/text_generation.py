...
from deepsparse import KVCache, Pipeline
...
class TextGenerationPipeline(TransformersPipeline):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cache_bools = []  # to be populated in overwrite_model_inputs
        ...
        self.engine_args["cache_bools"] = self.cache_bools
        ...
        self.kv_cache = KVCache({}, 0, [0])  # TODO: populate with correct values
    ...
    def engine_forward(
        ...
        """
        # get tokens by attention mask and setup kv_cache
        tokens = engine_inputs[0][engine_inputs[1].nonzero()].tolist()
        num_prompt_tokens = len(tokens) - 1
        self.kv_cache.reset(num_prompt_tokens, [0])  # prompt_length, frozen positions

        # run the prompt through
        tokens, kv_cache = self.prompt_inference(engine_inputs, tokens)
    ...
    def prompt_inference(
        self, engine_inputs: List[numpy.ndarray], tokens: List[int]
        ...
        :param tokens: list of raw tokens that are inputs in engine_inputs
        :return:
    ...
    def autoregressive_inference(
        ...
        new_logits, *cache_values = self.engine(engine_inputs, kv_cache=kv_cache)
        self.kv_cache.shift_last(1)
        # TODO: do we still need to update the cache below since engine handles?
        kv_cache = self.assemble_kv_cache(cache_values, tokens)
    ...
    def overwrite_onnx_model_inputs(
        ...
        # set cache_values bool list
        self.cache_bools = [
            external_input.name.startswith("past_key_values")
            for external_input in external_inputs
        ]
