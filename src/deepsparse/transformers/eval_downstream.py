...
def truncate(completion):
    # completion is solution generated by calling codegen
...
def benchmark_humaneval(model_path, dataset_name, n, temperatures):
    ...
    sys.path.append("/home/shubhra/workspace/codegen/deepsparse/")
    from examples.codegen.text_generation import TextGenerationPipeline

    os.environ["HF_ALLOW_CODE_EVAL"] = "1"
    os.environ["TOKENIZERS_PARALLELISM"] = "true"

    benchmark_problems_tokenlen = {"HumanEval/83": 46, "HumanEval/35": 76, "HumanEval/22": 95, "HumanEval/146": 112,
                                   "HumanEval/77": 121, "HumanEval/33": 143, "HumanEval/41": 160, "HumanEval/113": 178,
                                   "HumanEval/72": 236, "HumanEval/115": 334, "HumanEval/129": 428}

    orig_dataset = load_dataset(dataset_name, split="test")
    print("Creating Benchmark Dataset")
    benchmark_dataset = orig_dataset.filter(lambda x: x["task_id"] in benchmark_problems_tokenlen)

    print("Creating Codegen model with sequence length 240")
    codegen240 = TextGenerationPipeline(
    ...
    print("Creating Codegen model with sequence length 320")
    codegen320 = TextGenerationPipeline(
       model_path=model_path,
       engine_type="onnxruntime",
       deterministic=False,
       sampling_temperature=temperatures[0],
    ...
    print("Creating Codegen model with sequence length 428")
    codegen428 = TextGenerationPipeline(
       model_path=model_path,
       engine_type="onnxruntime",
       deterministic=False,
       sampling_temperature=temperatures[0],
    ...
    print("Creating Codegen model with sequence length 512")
    codegen512 = TextGenerationPipeline(
       model_path=model_path,
       engine_type="onnxruntime",
       deterministic=False,
       sampling_temperature=temperatures[0],
    ...
    print(f"Engine info: {codegen240.engine}")
    print(f"ORT Device: {ort.get_device()}\n")

    references = []
    predictions = []
    for idx, sample in _enumerate_progress(benchmark_dataset, len(benchmark_dataset)):
        ...
        if benchmark_problems_tokenlen[sample_task_id] <= 112:
            print("Using codegen240...")
            sample_predictions = codegen240(sequences=[sample_prompt] * n)
        elif benchmark_problems_tokenlen[sample_task_id] <= 192:
            print("Using codegen320...")
            sample_predictions = codegen320(sequences=[sample_prompt]*n)
        elif benchmark_problems_tokenlen[sample_task_id] <= 300:
            print("Using codegen428...")
            sample_predictions = codegen428(sequences=[sample_prompt]*n)
        else:
            print("Using codegen512...")
            sample_predictions = codegen512(sequences=[sample_prompt]*n)

        sample_predictions = sample_predictions.sequences
        for i in range(n):
            sample_predictions[i] = truncate(sample_predictions[i])
...
def human_eval(
    args, dataset_name="openai_humaneval", n=10, temperatures=[0.8], debug=False
):
    # n in the number of predictions to generate for each task where n >= k
    # (in the original paper k = [1, 10, 100], where k <= 100
    ...
    if args.benchmark_humaneval:
        return benchmark_humaneval(dataset_name="openai_humaneval", n=n, temperatures=temperatures)
    ...
    codegen = TextGenerationPipeline(
       model_path="/home/shubhra/workspace/codegen/codegen-350M-mono/",
       engine_type="onnxruntime",
       deterministic=False,
       sampling_temperature=temperatures[0],
       sequence_length=512,
    ...
    for idx, sample in _enumerate_progress(dataset, args.max_samples):
        ...
        print(f"sample_task_id: {sample_task_id}")
        print(f"\nsample_test: {sample_test}")
        predictions = codegen(sequences=[sample_prompt] * n)
        predictions = predictions.sequences
        for i in range(n):
            predictions[i] = truncate(predictions[i])
            print(predictions[i])

        sample_predictions.append(predictions[i])
        references.append(sample_test)
...
def _main(args):
    ...
    if dataset == "mnli":
        ...
        references, predictions = SUPPORTED_DATASETS[dataset](args)
        code_eval = load("code_eval")
        pass_at_k, results = code_eval.compute(references=references, predictions=predictions, k=[1, 10])
        print(f"\n{dataset} eval results: {pass_at_k}")
