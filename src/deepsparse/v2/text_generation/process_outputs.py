...
class ProcessOutputs(Operator):
    ...
    def run(
        ...
        generated_tokens = inp.get("generated_tokens")
        generated_logits = (
            inp.get("generated_logits") if generation_config.output_scores else None
        )
        finished_reason = [f[-1] for f in inp.get("finished_reason")]
