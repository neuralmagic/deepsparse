...
from deepsparse.server.helpers import log_system_metrics_server_level
...
def _add_endpoint(
    ...
    pipeline = Pipeline.from_config(pipeline_config, context, pipeline_logger)
...
def _add_pipeline_endpoint(
    ...
    def _predict(request: pipeline.input_schema):
        ...
        if pipeline.logger:
            log_system_metrics_server_level(pipeline.logger)
