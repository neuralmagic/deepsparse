...
DeepSparse allows accelerated inference, serving, and benchmarking of sparsified YOLACT models [YOLACT] ([original codebase](https://github.com/dbolya/yolact)
and [paper](https://arxiv.org/abs/1904.02689)).  
...
After training your model with `SparseML`, locate the `.pth` file for the model you'd like to export and run the `SparseML` integrated YOLOv5 ONNX export script below.
...
## Deployment Example
The following example uses pipelines to run a pruned and quantized YOLACT model for inference, downloaded by default from the SparseZoo. As input the pipeline ingests a list of images and returns for each image the detection boxes in numeric form. 
