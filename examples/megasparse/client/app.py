...
for model_name, model in feat.variants.items():
    ...
    if model_choice == model_name:
        ...
        context = st.text_area(label=feat.example_context_label, height=150, value=feat.example_context)
        question = st.text_area(label=feat.example_question_label, value=feat.example_question)
        start = perf_counter()
